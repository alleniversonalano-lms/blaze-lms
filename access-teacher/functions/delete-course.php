...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['course_id'])) {
    ...
    if ($stmt->num_rows === 0) {
        header("Location: ../dashboard?error=Unauthorized+deletion+attempt");
        exit;
    }
    ...
    if ($delete->execute()) {
        header("Location: ../dashboard?error=Course+deleted+successfully");
    } else {
        header("Location: ../dashboard?error=Failed+to+delete+course");
    }
    exit;
}
