...
logUserHistory("visited", "Groups"); // You can customize this per page
...
// Build section map: [user_id => ['Section A', 'Section B']]
$user_sections = [];

$stmt = $conn->prepare("
    SELECT sm.user_id, s.name AS section_name
...
while ($row = $result->fetch_assoc()) {
    $uid = $row['user_id'];
    $user_sections[$uid][] = $row['section_name'];
}
...
<body>
    ...
    <div class="main-content">
        ...
        <?php if ($user_id === $creator_id): ?>
            ...
            <div class="section-nav" style="margin-top: 20px; border-bottom: 2px solid #ccc; padding-bottom: 10px;">
                <a href="people" class="section-nav-tab" style="margin-right: 20px; text-decoration: none; color: #333;">People</a>
                <a href="groups" class="section-nav-tab active" style="text-decoration: none; font-weight: bold; color: #6c1d45;">Groups</a>
            </div>
        ...
        <?php if ($user_id === $creator_id): ?>
            ...
            <div class="nav-button">
                <button class="nav-btn" onclick="openModal()">Create Section</button>
            </div>
        <?php endif; ?>
        ...
        <div id="sectionModal" class="modal-overlay" style="display: none;">
            <div class="modal-content" style="max-width: 600px; width: 90%;">
                ...
                <!-- Create New Section -->
                <form method="post" action="create-section-handler" style="margin-bottom: 16px;">
                    <input type="text" name="section_name" placeholder="New Section Name" required style="width: 70%;">
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 20px;">
                        <?php foreach ($sections as $section): ?>
                            <div style="position: relative; display: inline-block;">
                                <!-- View Button -->
                                <button class="section-tab" onclick="showUsersForSection(<?= $section['id'] ?>)">
                                    <?= htmlspecialchars($section['name']) ?>
                                </button>

                                <!-- Delete Button (only course creator) -->
                                <?php if ($user_id === $creator_id): ?>
                                    <form method="post" action="delete-section-handler"
                                        onsubmit="return confirm('Are you sure you want to delete this section?');"
                                        style="position: absolute; top: -6px; right: -6px;">
                                        <input type="hidden" name="section_id" value="<?= $section['id'] ?>">
                                        <input type="hidden" name="course_id" value="<?= $course_id ?>">
                                        <button type="submit" class="delete-section-btn"
                                            style="background: red; color: white; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 14px; line-height: 20px; text-align: center; padding: 0; cursor: pointer;">
                                            ×
                                        </button>
                                    </form>
                                <?php endif; ?>
                ...
                <?php else: ?>
                    <p style="color: #666;">No sections created yet.</p>
                <?php endif; ?>



                <!-- User List Container (dynamically shown) -->
                <?php foreach ($sections as $section): ?>
                    <div class="section-user-list" id="section-<?= $section['id'] ?>" style="display: none;">
                        <form method="post" action="assign-section-handler">
                            <h4>Assign to <?= htmlspecialchars($section['name']) ?></h4>
                            <input type="hidden" name="section_id" value="<?= $section['id'] ?>">
                            <input type="hidden" name="course_id" value="<?= $course_id ?>">
                                ...
                                <?php foreach ($students as $student): ?>
                                    <div>
                                        <label>
                                            ...
                                            <?= htmlspecialchars($student['first_name'] . ' ' . $student['last_name']) ?> (<?= htmlspecialchars($student['email_address']) ?>)
                                        </label>
                                    </div>
                                <?php endforeach; ?>

                                <?php foreach ($lecturers as $lecturer): ?>
                                    <?php if ($lecturer['id'] !== $creator_id): ?>
                                        <div>
                                            <label>
                                                <input type="checkbox" name="user_ids[]" value="<?= $lecturer['id'] ?>">
                                                <?= htmlspecialchars($lecturer['first_name'] . ' ' . $lecturer['last_name']) ?> (Lecturer)
                                            </label>
                                        </div>
                                    <?php endif; ?>
                                <?php endforeach; ?>
                            ...
                            <button type="submit" class="modal-btn">Assign Selected</button>
                        </form>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <div style="margin-bottom: 20px;">
            <input type="text" id="searchInput" placeholder="Search by name or section..." style="padding: 8px; width: 100%; max-width: 300px;">
        </div>

        <script>
            document.getElementById('searchInput').addEventListener('input', function() {
                const filter = this.value.toLowerCase();
                const peopleTables = document.querySelectorAll('.people-table tbody');

                peopleTables.forEach(tbody => {
                ...
                <tbody>
                    <?php if ($creator_id == $user_id): ?>
                        <tr>
                            ...
                            <?php if ($user_id === $creator_id): ?>
                                <td>
                                    <?= isset($user_sections[$lecturer['id']])
                                        ? htmlspecialchars(implode(', ', $user_sections[$lecturer['id']]))
                ...
                <tbody>
                    <?php foreach ($students as $student): ?>
                        ...
                        <tr>
                            ...
                            <?php if ($user_id === $creator_id): ?>
                                <td>
                                    <?= isset($user_sections[$student['id']])
                                        ? htmlspecialchars(implode(', ', $user_sections[$student['id']]))
                                        : '<span style="color:#aaa;">—</span>' ?>
                                </td>
                            <?php endif; ?>
                        </tr>
    ...
    <script>
        ...
        function closeModal() {
            ...
            document.querySelectorAll('.section-user-list').forEach(el => el.style.display = 'none');
        }

        function showUsersForSection(sectionId) {
            document.querySelectorAll('.section-user-list').forEach(el => el.style.display = 'none');
