...
logUserHistory("visited", "Create Material"); // You can customize this per page
...
.unread-count {
        ...
        .form-card {
            ...
            /* space top and bottom, aligned to left */
            position: relative;
            z-index: 100;
        }
...
<body>
    ...
    <div class="main-content">
        ...
        if (isset($_POST['module_id'])) {
            $_SESSION['current_module_id'] = (int)$_POST['module_id'];
        } elseif (isset($_GET['module_id'])) {
            $_SESSION['current_module_id'] = (int)$_GET['module_id'];
        }

        // Fallback if not set
        if (!isset($_SESSION['current_module_id'])) {
            echo "<p style='padding: 20px; color: #e74c3c;'>Module ID is required.</p>";
            exit;
        }

        $module_id = $_SESSION['current_module_id'];
        ?>

        <div class="form-card">
            <form id="topicForm" action="functions/create-topic" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="module_id" value="<?= $module_id ?>">
                <input type="hidden" name="user_id" value="<?= $_SESSION['user_id'] ?>">

                <h3 class="form-header">New Material</h3>
                ...
                <div class="form-group">
                    ...
                    <input type="file" id="fileInput" name="attachments[]" multiple class="form-control" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.zip,.rar">
                    <div id="filePreview" style="margin-top: 10px;"></div>
                </div>

                <div class="form-group">
                    <label for="linkInputs">Attach Links (optional)</label>
                    <div id="linkInputs">
                        <input type="text" name="links[]" class="form-control" placeholder="https://example.com">
                    </div>
                    <button type="button" onclick="addLinkInput()" class="btn btn-sm" style="margin-top: 6px;">+ Add Another Link</button>
                </div>
