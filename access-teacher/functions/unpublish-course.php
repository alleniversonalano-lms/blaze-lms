...
$stmt = $conn->prepare("UPDATE courses SET published = 0 WHERE id = ? AND user_id = ?");
$stmt->bind_param("ii", $course_id, $user_id);

if ($stmt->execute() && $stmt->affected_rows > 0) {
    header("Location: ../dashboard?error=Course+unpublished+successfully");
    exit;
} else {
    header("Location: ../dashboard?error=Unable+to+unpublish+course+or+unauthorized+access");
    exit;
}
