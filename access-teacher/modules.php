...
logUserHistory("visited", "Modules"); // You can customize this per page
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="nav-button">
            <a href="create-module-page" class="nav-btn">Create Module</a>
        </div>
        ...
        <?php
        $stmt = $conn->prepare("SELECT * FROM modules WHERE course_id = ? ORDER BY created_at DESC");
        ...
        if ($result->num_rows === 0): ?>
            ...
            while ($row = $result->fetch_assoc()):
                ...
                $created_at = !empty($row['created_at']) ? date("F j, Y - g:i A", strtotime($row['created_at'])) : 'N/A';
                $scheduled_raw = $row['scheduled_at'];
                $scheduled_at = $scheduled_raw ? date("F j, Y - g:i A", strtotime($scheduled_raw)) : null;
                $is_published = (int)$row['is_published'];

                // Show all modules regardless of scheduling in proctor view
            ?>
                <div class="module-card">
                    <div class="module-header">
                        <div class="module-title">
                            <h3><?= $module_number ?>: <?= $title ?></h3>
                            <p class="module-description"><?= $description ?></p>
                            <br>
                            <?php if (!$is_published): ?>
                                <span style="display:inline-block; margin-top:6px; background:#e74c3c; color:white; padding:4px 8px; border-radius:4px; font-size:0.8rem;">Unpublished</span>
                            <?php endif; ?>
                        </div>
                        <div class="module-meta">
                            <?php if ($scheduled_at): ?>
                                <span class="module-date">Scheduled: <?= $scheduled_at ?></span><br>
                            <?php endif; ?>
                            <span class="module-date">Created: <?= $created_at ?></span>
                            <div class="menu-container">
                                <button class="menu-button">⋮</button>
                                <div class="dropdown-menu">
                                    <form action="edit-module-page" method="POST" style="display:inline;">
                                        <input type="hidden" name="module_id" value="<?= $module_id ?>">
                                        <button type="submit" class="dropdown-link">Edit</button>
                                    </form>
                                    <?php if ($is_published): ?>
                                        <a href="functions/unpublish-module?id=<?= $module_id ?>" class="dropdown-link">Unpublish</a>
                                    <?php else: ?>
                                        <a href="functions/publish-module?id=<?= $module_id ?>" class="dropdown-link">Publish</a>
                                    <?php endif; ?>

                                    <a href="functions/delete-module?id=<?= $module_id ?>" class="dropdown-link" onclick="return confirm('Are you sure you want to delete this module?')">Delete</a>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="module-footer">
                        <a href="view-module-stream?id=<?= $module_id ?>" class="view-btn">View Module</a>
                    </div>
                </div>
        <?php
            endwhile;
