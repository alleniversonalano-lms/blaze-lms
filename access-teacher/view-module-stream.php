...
logUserHistory("visited", "Module Stream"); // You can customize this per page
...
<body>
    ...
    <div class="main-content">
        ...
        $module_id = (int)$_GET['id']; // Make sure this line exists
        ?>

        <div class="nav-button">
            <form action="create-topic-page" method="POST">
                <input type="hidden" name="module_id" value="<?= $module_id ?>">
                <button type="submit" class="nav-btn">Create Material</button>
            </form>
        </div>
        ...
        if (isset($_GET['id'])) {
            $_SESSION['module_id'] = (int) $_GET['id'];
        }
                ...
                <div class="stream-card">
                    <div class="stream-header" style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div class="menu-container">
                            ...
                            <?php if (!$is_published): ?>
                                <span style="font-size: 12px; color: #e74c3c; font-weight: bold;">Unpublished</span>
                            <?php endif; ?>
                            <div class="stream-timestamp"><?= $created_at ?></div>
                            <button class="menu-button">⋮</button>
                            <div class="dropdown-menu">
                                <form action="edit-topic-page" method="POST">
                                    <input type="hidden" name="id" value="<?= $stream_id ?>">
                                    <button type="submit" class="dropdown-link">Edit</button>
                                </form>
                                <form action="functions/toggle-publish" method="POST">
                                    <input type="hidden" name="id" value="<?= $stream_id ?>">
                                    <button type="submit" class="dropdown-link"><?= $is_published ? 'Unpublish' : 'Publish' ?></button>
                                </form>
                                <form action="functions/delete-topic" method="POST" onsubmit="return confirm('Are you sure you want to delete this topic?');">
                                    <input type="hidden" name="id" value="<?= $stream_id ?>">
                                    <button type="submit" class="dropdown-link">Delete</button>
                                </form>
                            </div>
                        </div>
