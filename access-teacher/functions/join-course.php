...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($entered_code === '') {
        header("Location: ../collaboration?error=Class+code+is+required");
        exit;
    }
    ...
    if ($result->num_rows === 0) {
        header("Location: ../collaboration?error=Invalid+class+code");
        exit;
    }
    ...
    if ($creator_id == $user_id) {
        header("Location: ../collaboration?error=You+are+already+the+creator+of+this+course");
        exit;
    }
    ...
    if ($role === 'teacher') {
        ...
        if ($check_result->num_rows > 0) {
            header("Location: ../collaboration?error=Already+joined+this+course+as+teacher");
            exit;
        }
        ...
        if ($check_result->num_rows > 0) {
            header("Location: ../collaboration?error=Already+enrolled+in+this+course");
            exit;
        }
        ...
        header("Location: ../collaboration?error=Unknown+user+role");
        exit;
    }

    // Attempt to execute the insert
    if ($insert->execute()) {
        header("Location: ../collaboration?error=Successfully+joined+the+course");
        exit;
    } else {
        header("Location: ../collaboration?error=Failed+to+join+course");
        exit;
    }
} else {
    header("Location: ../collaboration");
    exit;
}
