...
try {
    ...
    $quiz_data = json_decode($assessment['quiz_data'], true);
    ...
    $js_quiz_data = [
        ...
        'allowedAttempts' => $assessment['allowed_attempts'] ?? 1,
        'currentAttempt' => 1,
        'scoreToKeep' => $assessment['score_to_keep'] ?? 'highest',
        'shuffleQuestions' => (bool)($assessment['shuffle_questions'] ?? false),
        'shuffleAnswers' => (bool)($assessment['shuffle_answers'] ?? false),
        'questions' => $questions ?? []
    ];

    if (empty($js_quiz_data['questions'])) {
        throw new Exception("No questions found for this assessment");
    }
} catch (Exception $e) {
    header("Location: assessments?error=" . urlencode($e->getMessage()));
    exit;
}
?>
